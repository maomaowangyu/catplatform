        ...
        $(document).ready(function() {
            $(".checkboxcss").click(function (){
                if ($(this).is(':checked')) {
                    ...
                    $("#"+$(this).attr("id")+"t").val("1");
                } else {
                    //$(this).removeAttr("checked");
                    $("#"+$(this).attr("id")+"t").val("0");
                }
            });
                                            ...
                                            <table id="contentTable"  class="table table-striped table-bordered table-condensed">
                                                ...
                                                <c:forEach items="${genTable.columnList}" var="column" varStatus="vs">
                                                    <tr${column.delFlag eq '1'?' class="error" title="已删除的列，保存之后消失！"':''}>
                                                        ...
                                                        <td>
                                                            <input type="checkbox" id="${vs.index}isPk" name="columnList[${vs.index}].isPks" class="checkboxcss" value="1" ${column.isPk eq '1' ? 'checked' : ''}/>
                                                            <input type="hidden" id="${vs.index}isPkt" name="columnList[${vs.index}].isPk" value="${column.isPk}"/>
                                                        </td>
                                                        <td>
                                                            <input type="checkbox" id="${vs.index}isNull" name="columnList[${vs.index}].isNulls" class="checkboxcss" value="1" ${column.isNull eq '1' ? 'checked' : ''}/>
                                                           <input type="hidden" id="${vs.index}isNullt" name="columnList[${vs.index}].isNull"  value="${column.isNull}" />
                                                        </td>
                                                        <td>
                                                            <input type="checkbox" id="${vs.index}isInsert"  name="columnList[${vs.index}].isInserts" class="checkboxcss" value="1" ${column.isInsert eq '1' ? 'checked' : ''}/>
                                                            <input type="hidden" id="${vs.index}isInsertt" name="columnList[${vs.index}].isInsert" value="${column.isInsert}"/>
                                                        </td>
                                                        <td>
                                                            <input type="checkbox" id="${vs.index}isEdit"  name="columnList[${vs.index}].isEdits" class="checkboxcss" value="1" ${column.isEdit eq '1' ? 'checked' : ''}/>
                                                            <input type="hidden" id="${vs.index}isEditt" name="columnList[${vs.index}].isEdit" value="${column.isEdit}"/>
                                                        </td>
                                                        <td>
                                                            <input type="checkbox" id="${vs.index}isList"  name="columnList[${vs.index}].isLists" class="checkboxcss" value="1" ${column.isList eq '1' ? 'checked' : ''}/>
                                                            <input type="hidden" id="${vs.index}isListt" name="columnList[${vs.index}].isList" value="${column.isList}"/>
                                                        </td>
                                                        <td>
                                                            <input type="checkbox" id="${vs.index}isQuery" name="columnList[${vs.index}].isQuerys" class="checkboxcss" value="1" ${column.isQuery eq '1' ? 'checked' : ''}/>
                                                            <input type="hidden" id="${vs.index}isQueryt" name="columnList[${vs.index}].isQuery" value="${column.isQuery}"/>
                                                        </td>
                                                        <td>
